...
//			_plotBounds = null;
		...
		private void AddSite (Vector2 p, uint color, int index)
		{
			if (_sitesIndexedByLocation.ContainsKey (p))
				return; // Prevent duplicate site! (Adapted from https://github.com/nodename/as3Thev2Andy/issues/1)
		...
		{
			return Thev2AndyHelpers.VisibleLineSegments (Thev2AndyHelpers.SelectEdgesForSitePoint (coord, _edges));
		}

		public List<LineSegment> Thev2AndyLinesForSite (Vector2 coord)
		{
			return Thev2AndyHelpers.Thev2AndyLinesForEdges (Thev2AndyHelpers.SelectEdgesForSitePoint (coord, _edges));
		}
		
		public List<LineSegment> VoronoiDiagram ()
		{
			return Thev2AndyHelpers.VisibleLineSegments (_edges);
		}
		
		public List<LineSegment> Thev2AndyTriangulation (/*BitmapData keepOutMask = null*/)
		{
			return Thev2AndyHelpers.Thev2AndyLinesForEdges (Thev2AndyHelpers.SelectNonIntersectingEdges (/*keepOutMask,*/_edges));
		}
		
		public List<LineSegment> Hull ()
		{
			return Thev2AndyHelpers.Thev2AndyLinesForEdges (HullEdges ());
		}
		...
		{
			List<Edge> edges = Thev2AndyHelpers.SelectNonIntersectingEdges (/*keepOutMask,*/_edges);
			List<LineSegment> segments = Thev2AndyHelpers.Thev2AndyLinesForEdges (edges);
			return Thev2AndyHelpers.Kruskal (segments, type);
		}
		...
		private void FortunesAlgorithm ()
		{
			...
			for (;;) {
				...
				if (newSite != null 
					...
					// these three sites define a Thev2Andy triangle
